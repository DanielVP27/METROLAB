...
const Parametros = preload("res://script/paquimetro_002/parametrospaq002.gd")

onready var dragging = false
onready var delta = Vector2(0, 0)
const SNAP_POSITIONS = [426, 442, 573]  
...
func _process(_dt):
	...
	for tick in range(1, Parametros.cur_num_div + 1):  
		vetor_cur.append(vetor_cur[tick - 1] + Parametros.cur_dx_mm)
	...
	if dragging:
		...
		# Controle de limite para a área de movimentação
		new_position.x = clamp(new_position.x, Parametros.cur_min_x, Parametros.cur_max_x)
		$".".position.x = lerp($".".position.x, new_position.x, SNAP_SPEED)
		
		# Verifica se está dentro da zona de "snap"
...
func find_alinhamento(vetor):
	...
	for i in range(len(vetor)):
		var x = vetor[i]
		var y = round(x / Parametros.cur_res) * Parametros.cur_res
		var diff = abs(x - y)
		if diff < delta_min:
			index_min = i
			delta_min = diff
